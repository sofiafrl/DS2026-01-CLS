...
A interface oficial do protótipo é a página estática `index.html` localizada na raiz do projeto, que pode ser servida por qualquer servidor estático ou pelo script local.
...
- `app.js`: coordenação da interface estática.
- `musicService.js`: orquestração e execução local da interpretação e geração de MIDI.
- `audioPlayer.js`: reprodução no navegador com Web Audio API.
- `fileService.js`: leitura de TXT e downloads.
- `pieceRenderer.js`: renderização do resumo e dos eventos musicais.
